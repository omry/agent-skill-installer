...
def require_wheelhouse_requirement(package: str, find_links: Path) -> None:
    try:
        requirement = Requirement(package)
    except InvalidRequirement as error:
        raise InstallerError(
            "external wheel package must be a name and version requirement "
            f"to resolve from {find_links}: {package}"
        ) from error
    if requirement.url is not None:
        raise InstallerError(
            "external wheel package must not use a direct reference "
            f"to resolve from {find_links}: {package}"
        )
    if is_distribution_archive_filename(package):
        raise InstallerError(
            "external wheel package must be a distribution name, not a filename, "
            f"to resolve from {find_links}: {package}"
        )


def is_distribution_archive_filename(value: str) -> bool:
    try:
        parse_wheel_filename(value.strip())
    except InvalidWheelFilename:
        pass
    else:
        return True
    try:
        parse_sdist_filename(value.strip())
    except InvalidSdistFilename:
        return False
    return True


def wheelhouse_pip_environment() -> dict[str, str]:
    return {**os.environ, "PIP_CONFIG_FILE": os.devnull}
...
def run_pip_wheel(
    *,
    ...
    if find_links is not None:
        require_wheelhouse_requirement(package, find_links)
        command.extend(
            [
                "--isolated",
                "--no-index",
    ...
    try:
        if find_links is None:
            ...
            with tempfile.TemporaryDirectory(
                prefix="agent-skill-installer-pip-cwd-"
            ) as isolated_cwd:
                completed = subprocess.run(
                    command,
                    cwd=isolated_cwd,
                    ...
                    capture_output=True,
                    check=False,
                    env=wheelhouse_pip_environment(),
                )
    except OSError as error:
...
def apply_uninstall_hook(
    spec: TargetSpec,
    plan: tuple[str, bool] | None,
    *,
    delete_if_empty: bool,
) -> bool:
    if plan is None:
        return False
    updated, changed = plan
    if not changed:
        return False
...
def uninstall_target(project: SkillProject, spec: TargetSpec) -> InstallResult:
    ...
    instruction_hook_plan = plan_uninstall_hook(
        spec,
    ...
    apply_uninstall_hook(
        spec,
        instruction_hook_plan,
