...
# Absolute paths in prose look exactly like Claude Code triggers.
_PATH_ROOT_TOKENS = frozenset(
    {
        "bin",
        "dev",
        "etc",
...
def is_advertised_trigger(text: str, start: int) -> bool:
    ...
    Codex bodies use ``$`` for shell variables too, so a bare ``$output`` in
    the middle of a sentence is not a trigger. Only a token that opens a line
    or follows a cue word such as "use" is treated as advertised.
    """
...
def trigger_tokens(text: str, sigil: str) -> list[str]:
    ...
    for match in pattern.finditer(text):
        ...
        if not token or _PLACEHOLDER_TOKEN.fullmatch(token) or token in tokens:
            continue
        if sigil == "/" and (token in _PATH_ROOT_TOKENS or "." in token):
            continue
        if sigil == "$" and not is_advertised_trigger(text, match.start()):
            continue
        tokens.append(token)
    return tokens
