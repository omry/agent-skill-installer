...
# A complete Markdown line prefix: list marker, heading, quote, or table cell.
_LINE_PREFIX = re.compile(r"\s*(?:[-*+>|]+|#{1,6}|\d+[.)])?\s*")
# Text joining coordinated triggers, as in "Use /one or /two".
_COORDINATOR = re.compile(r"[\s,;|/`'\"()\[\]-]*(?:or|and|plus)?[\s,;|/`'\"()\[\]-]*")
...
def is_advertised_trigger(text: str, start: int) -> bool:
    ...
    # Only a complete line prefix opens a line; trailing sentence punctuation
    # such as "Store the result: $output" does not.
    if _LINE_PREFIX.fullmatch(before):
        return True
...
def trigger_tokens(text: str, sigil: str) -> list[str]:
    ...
    for match in pattern.finditer(text):
        ...
        if sigil == "$" and _PLACEHOLDER_TOKEN.fullmatch(token):
            continue
        advertised = is_advertised_trigger(text, match.start())
        if not advertised and advertised_end is not None:
            # "Use /one or /two" advertises both, so carry the context along.
            advertised = bool(
                _COORDINATOR.fullmatch(text[advertised_end : match.start()])
            )
        if not advertised:
            continue
        advertised_end = match.end()
