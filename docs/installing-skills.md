...
`python -m pip wheel --no-deps --no-index --only-binary=:all: --find-links <dir>`
for the declared spec, so no index is consulted and no source distribution is
built. Place both artifacts in one directory:
