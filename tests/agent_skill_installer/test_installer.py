...
def test_trigger_tokens_keep_path_shaped_skill_names(tmp_path: Path) -> None:
    # Skill names may look like paths, so shape alone cannot exclude a token.
    assert trigger_tokens("Use /run to launch the app", "/") == ["run"]
    assert trigger_tokens("Use /other.name for this skill", "/") == ["other.name"]

    # A cued path does surface, but naming the skill's own trigger suppresses
    # the warning, which is what well-formed bodies do.
    project = replace(
        make_project(tmp_path),
        hook_blocks={
            "claude": "Write scratch output to /tmp. Use `/example-agent-skill` to start."
        },
    )
    assert trigger_mismatch_warning(project, "claude") is None


def test_trigger_tokens_keep_uppercase_claude_commands() -> None:
    assert trigger_tokens("Use /OTHER for this skill", "/") == ["OTHER"]
    assert trigger_tokens("Use $PATH here", "$") == []
    assert trigger_tokens("Use $ARGUMENTS in the body", "$") == []


def test_trigger_tokens_read_markdown_prefixed_line_starts() -> None:
    assert trigger_tokens("* /other for this skill", "/") == ["other"]
    assert trigger_tokens("+ /other for this skill", "/") == ["other"]
    assert trigger_tokens("## /other", "/") == ["other"]
    assert trigger_tokens("| /other | notes |", "/") == ["other"]
    assert trigger_tokens("1. /other", "/") == ["other"]
    assert trigger_tokens("* $other for this skill", "$") == ["other"]


def test_trigger_tokens_ignore_sentence_punctuation_before_token() -> None:
    assert trigger_tokens("Store the result: $output", "$") == []
    assert trigger_tokens("Files are located at: /workspace", "/") == []
    assert trigger_tokens("Do that. /workspace holds it", "/") == []
    assert trigger_tokens("Intro line.\nUse /awd for plans.", "/") == ["awd"]
    assert trigger_tokens("## /other", "/") == ["other"]


def test_trigger_tokens_read_coordinated_trigger_lists(tmp_path: Path) -> None:
    assert trigger_tokens("Use /other or /actual", "/") == ["other", "actual"]
    assert trigger_tokens("Use /a, /b and /c", "/") == ["a", "b", "c"]
    assert trigger_tokens("Use `/a` or `/b`", "/") == ["a", "b"]

    # Context must not leak past a coordinator into ordinary prose.
    assert trigger_tokens("Use /awd for plans; files are in /workspace", "/") == ["awd"]
    assert trigger_tokens("Use /awd. Store output at: /workspace", "/") == ["awd"]

    # A body naming its own trigger in a list must not warn.
    project = replace(
        make_project(tmp_path),
        hook_blocks={"claude": "Use /other or /example-agent-skill for this."},
    )
    assert trigger_mismatch_warning(project, "claude") is None
