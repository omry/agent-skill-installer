...
def dispatch_publish(root: Path, config: ReleaseConfig, commit: str) -> str:
    ...
    return f"https://github.com/{REPOSITORY}/actions/workflows/publish.yml"
...
def print_dispatch_result(config: ReleaseConfig, commit: str, run_url: str) -> None:
    ...
    if config.publish:
        if "/actions/runs/" in run_url:
            print("Approval page (available after a quick PyPI status check):")
        else:
            print(
        ...
        print(
            ...
            "then approve and deploy. The approval appears before full "
            "validation finishes; publishing starts only after both approval "
        ...
        label = (
            "Workflow run"
            if "/actions/runs/" in run_url
            else "Publish workflow runs"
        )
        print(f"{label}: {run_url}")
